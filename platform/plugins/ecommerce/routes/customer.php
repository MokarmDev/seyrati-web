...
Theme::registerRoutes(function (): void {
    Route::group([
        ...
        // Old password reset routes (kept for backward compatibility)
        Route::post('password/email', 'ForgotPasswordController@sendResetCode')->name('password.request');
        Route::get(
            EcommerceHelper::getPageSlug('reset_password'),
            'ForgotPasswordController@showLinkRequestForm'
        )->name('password.reset');

        // New password reset OTP routes
        Route::get('password/reset/otp/verify', 'PasswordResetOtpController@showVerifyForm')->name('password.reset.otp.verify');
        Route::post('password/reset/otp/verify', 'PasswordResetOtpController@verify')->name('password.reset.otp.verify.post');
        Route::post('password/reset/otp/resend', 'PasswordResetOtpController@resend')->name('password.reset.otp.resend');

        // Password update routes
        Route::get('password/reset/new-password', 'PasswordResetUpdateController@showForm')->name('password.reset.form');
        Route::post('password/reset/new-password', 'PasswordResetUpdateController@update')->name('password.reset.update');

        // Keep old route for potential compatibility (will redirect to OTP verification)
        ...
        Route::post('otp/verify', 'OtpController@verify')->name('otp.verify.post');
