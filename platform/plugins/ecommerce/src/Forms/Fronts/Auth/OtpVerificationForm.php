...
use Botble\Ecommerce\Http\Requests\Fronts\Auth\OtpVerificationRequest;

class OtpVerificationForm extends AuthForm
{
    public static function formTitle(): string
    {
        return __('OTP verification form');
    }
    ...
    public function setup(): void
    {
        ...
        $this
            ->setUrl(route('customer.otp.verify.post'))
            ->setValidatorClass(OtpVerificationRequest::class)
            ->icon('ti ti-shield-check')
            ->heading(__('التحقق من الهاتف'))
            ...
            ->add('resend_otp', HtmlField::class, [
                'html' => sprintf(
                    '<div class="mt-3 text-center">
                        <button type="button" onclick="resendOtp(\'%s\')" class="text-decoration-underline p-0" style="border:none; background:none; cursor:pointer; font-size:inherit; color:#0066cc;" id="resendBtn">%s</button>
                    ...
                    route('customer.otp.resend'),
                    __('إعادة إرسال الرمز')
                ),
            ])
            ->add('back_to_register', HtmlField::class, [
                'html' => sprintf(
