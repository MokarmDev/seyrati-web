...
class Typography
{
    ...
    public function renderCssVariables(): string
    {
        if (empty($this->fontFamilies)) {
            $fontFamily = new TypographyItem('primary', trans('packages/theme::theme.typography.primary'), theme_option('primary_font', 'Inter'));
        ...
        Event::listen(RenderingThemeOptionSettings::class, function (): void {
            ...
            foreach ($this->fontFamilies as $fontFamily) {
                $fields[] = GoogleFontsField::make()
                    ...
                    ->label(trans('packages/theme::theme.typography.font_family', ['name' => $fontFamily->getLabel()]))
            ...
            foreach ($this->fontSizes as $fontSize) {
                $fields[] = NumberField::make()
                    ...
                    ->label(trans('packages/theme::theme.typography.font_size', ['name' => $fontSize->getLabel()]))
                    ->defaultValue($fontSize->getDefault())
                    ->helperText(trans('packages/theme::theme.typography.font_size_helper', [
